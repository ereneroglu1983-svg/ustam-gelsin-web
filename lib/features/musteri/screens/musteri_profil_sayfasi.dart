...
class _MusteriProfilSayfasiState extends State<MusteriProfilSayfasi> {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(colors: [Color(0xFF0F4C81), Color(0xFF0A3356)], begin: Alignment.topLeft, end: Alignment.bottomRight),
                    ...
                    child: const Row(
                      ...
                      children: [
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Hemen Usta Bul", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), SizedBox(height: 5), Text("Yeni bir ilan oluşturun", style: TextStyle(color: Colors.white70, fontSize: 13))]),
              ...
              Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(colors: [Color(0xFFBA1A1A), Color(0xFF680003)], begin: Alignment.topLeft, end: Alignment.bottomRight),
                      borderRadius: BorderRadius.circular(20),
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text("7/24 ACİL USTA", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold, letterSpacing: 0.8)), SizedBox(height: 4), Text("Tesisat, Elektrik, Çilingir, Klima...", style: TextStyle(color: Colors.white70, fontSize: 12))]),
                        SizedBox(width: 60, height: 60, child: Image.asset("assets/images/acil_usta_logo.png", fit: BoxFit.contain)),
                      ],
                    ),
                  ),
                ),
              ...
              _menuItem(icon: Icons.assignment_outlined, baslik: "İlanlarım", altBaslik: "Verilen ilanlar ve gelen teklifler", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const MusteriIlanlarimSayfasi()))),
              _menuItem(isAcil: true, icon: Icons.warning_amber_rounded, baslik: "Acil Usta İlanlarım", altBaslik: "Anlık acil çağrılarınızın durumu", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const MusteriAcilIlanlarim()))),
              _menuItem(icon: Icons.chat_bubble_outline, baslik: "Mesajlarım", altBaslik: "Ustalar ile görüşmeleriniz", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const MesajlarimSayfasi()))),
  ...
  Widget _menuItem({IconData? icon, required String baslik, required String altBaslik, required VoidCallback onTap, bool isAcil = false}) {
    return ListTile(
      onTap: onTap,
      leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(10)), child: isAcil ? Image.asset("assets/images/acil_usta_logo.png", width: 24, height: 24) : Icon(icon, color: const Color(0xFF0F4C81))),
      title: Text(baslik, style: TextStyle(color: isAcil ? Colors.redAccent : Colors.white, fontWeight: FontWeight.bold)),
  ...
  Widget _buildMesajBildirimHanesi(String? userId, BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('chats').where('aliciId', isEqualTo: userId).where('okundu', isEqualTo: false).snapshots(),
      builder: (context, snapshot) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(color: Colors.orange.withOpacity(0.15), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.orange.withOpacity(0.5), width: 1)),
            child: Row(children: [const Icon(Icons.mark_chat_unread, color: Colors.orange), const SizedBox(width: 15), Expanded(child: Text("$mesajSayisi adet yeni mesajınız var!", style: const TextStyle(color: Colors.orange, fontWeight: FontWeight.w600))), const Icon(Icons.chevron_right, color: Colors.orange)]),
          ),
        );
      },
    );
