// lib/features/musteri/screens/musteri_aktif_ilanlar_listesi.dart
...
class MusteriAktifIlanlarListesi extends StatelessWidget {
  const MusteriAktifIlanlarListesi({super.key});
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text("Henüz aktif ilanınız yok.", style: TextStyle(color: Colors.white70)));
        }
  ...
  Widget _buildIlanKarti(BuildContext context, IlanModel ilan, String docId) {
    bool isAcil = ilan.teknikDetaylar['isAcil'] == true;

    List<String> detaylar = [];
    if (!isAcil && ilan.teknikDetaylar.isNotEmpty) {
    ...
    Color durumRenk = isAcil ? Colors.white : Colors.green;
    String durumText = isAcil ? "ACİL ÇAĞRI" : "YAYINDA";

    return Container(
      margin: const EdgeInsets.only(bottom: 15),
      decoration: BoxDecoration(
        color: isAcil ? Colors.red.shade700.withOpacity(0.6) : Colors.white.withOpacity(0.05),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: isAcil ? Colors.redAccent : Colors.white10),
      ),
      child: Column(
        children: [
          InkWell(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Expanded(child: Text(ilan.baslik.toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16))),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                        decoration: BoxDecoration(color: durumRenk.withOpacity(0.3), borderRadius: BorderRadius.circular(4)),
                  ...
                  const SizedBox(height: 12),
                  if (isAcil) ...[
                    const Text("SORUN DETAYI:", style: TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.bold)),
                    Text(ilan.detaylar, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w500)),
                    const SizedBox(height: 12),
                  ] else if (detaylar.isNotEmpty) ...[
