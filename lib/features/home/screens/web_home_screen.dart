...
class _WebHomeScreenState extends State<WebHomeScreen> {
  ...
  Future<void> _initializeLocation() async {
    // ARTIK TEK KOD - hem web hem mobil için geolocator yeterli
  ...
  Future<void> _determinePosition() async {
    try {
      ...
      if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) { _setFallback(); return; }
      Position position = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.medium).timeout(const Duration(seconds: 5));
