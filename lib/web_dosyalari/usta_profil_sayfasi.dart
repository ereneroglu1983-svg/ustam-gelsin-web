// lib/web_dosyalari/usta_profil_sayfasi.dart
...
class _UstaProfilSayfasiState extends State<UstaProfilSayfasi> {
  ...
  Future<void> _resimGuncelle(String uid) async {
    final XFile? secilenDosya = await ImagePicker().pickImage(source: ImageSource.gallery);
    if (secilenDosya!= null) {
      final bytes = await secilenDosya.readAsBytes();
  ...
  void _show724InfoDialog(bool is724Active) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "Online olduğunuzda konumunuz güncellenir ve yakın bölgenizdeki çağrılar size düşer.",
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, userSnapshot) {
          ...
          final uzmanlikData = userData?['uzmanliklar']?? userData?['uzmanlikAlani']?? "Genel";
          _updateUstaBildirimBilgisi(user.uid, uzmanlikData);
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _menuItem(icon: Icons.account_balance_wallet_outlined, baslik: "Hesabım", altBaslik: "Kredi satın al ve harcama geçmişini gör", onTap: () => _cuzdanSayfasiniAc(context, user.uid)),
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData) {
          for (var doc in snapshot.data!.docs) {
            ...
            String ustaId = (data['atananUstaId']?? '').toString();
            String durum = (data['durum']?? '').toString();

            if (ustaId == uid) {
              if (durum == 'teklif_bekliyor') {
                bekleyen++;
              } else if (durum == 'is_verildi' || durum == 'devam_ediyor') {
                devamEden++;
          ...
          Row(
            children: [
              Expanded(
                child: Column(
                  children: [
                    const Text("Kredi", style: TextStyle(fontSize: 12, color: Colors.grey)),
                    const SizedBox(height: 5),
