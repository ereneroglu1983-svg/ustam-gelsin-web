...
import 'package:ustam_gelsin/core/services/billing_service.dart';
  ...
  Future<void> processOffer({
    ...
    try {
      ...
      if (durum == 'onaylandi') {
        ...
        // REVİZE: Önce offer içindeki mevcut tokenı kontrol et, yoksa Firebase'e git (Maliyet Kalkanı)
        String? musteriToken = offer['musteriFcmToken'];

        if (musteriToken == null) {
          final musteriDoc = await FirebaseFirestore.instance.collection('users').doc(musteriUserId).get();
          musteriToken = musteriDoc.data()?['fcmToken'];
        }
