// app/[city]/[job]/page.tsx - FINAL v4 - BUILD FIX + FAIL-FAST
...
function validateBuildData() {
  if (cities.length!== 81) throw new Error(`[BUILD FAIL] cities.length=${cities.length}; tam olarak 81 il olmalı`)
  const slugs = cities.map(c => c.slug)
  const uniqueSlugs = new Set(slugs)
  if (uniqueSlugs.size!== slugs.length) throw new Error(`[BUILD FAIL] cities.ts içinde duplicate slug var`)
  const mapKeys = Object.keys(nearbyMap)
  if (mapKeys.length!== 81) throw new Error(`[BUILD FAIL] nearbyMap ${mapKeys.length} şehir içeriyor, 81 olmalı`)
  for (const citySlug of slugs) {
    if (!nearbyMap[citySlug]) throw new Error(`[BUILD FAIL] nearbyMap içinde ${citySlug} anahtarı eksik`)
  }
  const citySlugSet = new Set(slugs)
  for (const [city, nearbyList] of Object.entries(nearbyMap)) {
    if (nearbyList.includes(city)) throw new Error(`[BUILD FAIL] ${city} kendisini yakın şehir olarak gösteriyor`)
    if (new Set(nearbyList).size!== nearbyList.length) throw new Error(`[BUILD FAIL] ${city} nearbyMap içinde duplicate var`)
    for (const n of nearbyList) {
      if (!citySlugSet.has(n)) throw new Error(`[BUILD FAIL] nearbyMap: ${city} -> ${n} geçersiz slug`)
    }
  }
}
...
export async function generateMetadata({params}:{params: Promise<{city:string,job:string}>}): Promise<Metadata>{
  ...
  return {
    ...
    twitter: { card: 'summary_large_image', title: seoData.metaTitle, description: seoData.metaDescription },
...
function getNearbyCities(currentSlug: string) {
  ...
  return cities.filter(c => nearbySlugs.includes(c.slug)).slice(0, 12)
}
...
export default async function JobCityPage({params}:{params: Promise<{city:string,job:string}>}){
  ...
  const firstIlce = seoData.ilceler?.[0] || city.name
  const ilceCount = seoData.ilceler?.length || 0
  ...
  const serviceSchema = {
    ...
    "description": seoData.metaDescription,
    "provider": { "@id": "https://hemenustamgelsin.com/#organization" },
    "areaServed": [
      { "@type": "City", "name": city.name },
   ...seoData.ilceler.map((d: string) => ({ "@type": "AdministrativeArea", "name": d }))
    ],
    "url": pageUrl
  }
      ...
      <section style={{ background: `radial-gradient(800px 400px at 15% 0%, ${color}15 0%, transparent 60%), #FFFBF5`, padding:'26px 20px 28px' }}>
        <div style={{maxWidth:1120, margin:'0 auto', display:'grid', gridTemplateColumns:'1.15fr 0.85fr', gap:24}}>
          <div>
            ...
            <h1 style={{fontSize:'clamp(28px, 4vw, 48px)', fontWeight:900, lineHeight:0.92, margin:0}}>{seoData.h1}<br/> HugAI Destekli Keşif ve Fiyat Tahmini</h1>
            ...
            <div style={{marginTop:16, background:'white', border:'1px solid #e7e5e4', borderRadius:12, padding:'12px 14px', display:'flex', gap:10}}>
              ...
              <div>
                <div style={{fontWeight:800, fontSize:13}}>{seoData.fiyatBilgisi} • Yaklaşık {seoData.fiyatAraligi}</div>
                <div style={{fontSize:11, color:'#78716c'}}>{seoData.fiyatBilgisi}</div>
            ...
            <div style={{marginTop:12, background:'#fffbeb', border:'1px solid #fde68a', borderRadius:12, padding:12}}>
              ...
              <div style={{fontSize:11, color:'#92400e', marginTop:6, lineHeight:1.5}}>
                ...
                2. {city.name} için {seoData.fiyatBilgisi} kontrol eder<br/>
                3. Yaklaşık maliyeti çıkarır: {seoData.fiyatAraligi}<br/>
                4. Ustalar komisyonsuz teklif verir
              </div>
            </div>
          </div>
          <div style={{background:'white', borderRadius:18, border:'1px solid #e7e5e4', padding:16}}>
            ...
            <div style={{display:'grid', gap:10}}>
              ...
              <div style={{background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:12, padding:'12px 10px'}}>
                ...
                <div style={{fontSize:11, color:'#166534', marginTop:2}}>HugAI fotoğraflarını analiz eder, {city.name} şartlarına göre yaklaşık {seoData.fiyatAraligi} tahminini çıkarır.</div>
              </div>
              <div style={{background:'#fafaf9', borderRadius:12, padding:'12px 10px'}}>
                <div style={{fontWeight:800, fontSize:12}}>3. Usta Eşleştirme ve Teklif</div>
                <div style={{fontSize:11, color:'#57534e', marginTop:2}}>{city.name} {firstIlce} dahil ustalar teklif verir, uygun usta ile iletişime geçebilirsin.</div>
              </div>
            </div>
      ...
      <section style={{maxWidth:1120, margin:'0 auto', padding:'18px 20px 60px', display:'grid', gridTemplateColumns:'1.2fr 0.8fr', gap:18}}>
        <div style={{background:'white', border:'1px solid #e7e5e4', borderRadius:16, padding:18}}>
          ...
          <ul style={{fontSize:13, color:'#44403c', lineHeight:1.6}}>
            <li>{city.name} {firstIlce} için HugAI analizli fiyat - {seoData.fiyatBilgisi}</li>
            <li>{city.name} genelinde {ilceCount} ilçede usta eşleştirme ve teklif alma</li>
            <li>{city.name} bölgesinde usta eşleştirme ve teklif alma</li>
            <li>%0 başarı komisyonu - Ustanın hakedişinden komisyon alınmaz</li>
